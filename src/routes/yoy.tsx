...
import {
  CHANNEL_REGIONS, ALL_REGIONS, REGION_COLOR, fullName, CHANNEL_ORDER,
  type ChannelKey,
} from "@/lib/regions";
...
function ChartView({
  ...
  // 构建数据
  const data: DataPoint[] = xs.map((x, i) => {
      ...
      for (const r of regions) {
        ...
        if (kpi.showYoY && i > 0) {
          ...
          const prevRow = kpi.source === "yoy"
            ? yoy.find((d) => String(d.pay_yr) === prevX && d.consume_large_area_name === fullName(r))
            : rep.find((d) => d.stat_month === prevX && d.consume_large_area_name === fullName(r));
          const pv = getVal(prevRow, s);
          point[`${r}__${s.key}__yoy`] = v != null && pv != null && pv !== 0 ? (v - pv) / pv : null;
        } else if (kpi.showYoY) {
          point[`${r}__${s.key}__yoy`] = null;
        }
      }
    } else {
      const r = regions[0];
